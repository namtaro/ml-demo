...
class KeyedInterfaceViewController: UIViewController {
    ...
    @IBAction func getBatteryLevel(_ sender: Any) {
        ...
        if isListeningBattery {
            ...
            DJISDKManager.keyManager()?.stopListening(on: batteryLevelKey!, ofListener: self)
            self.getBatteryLevelLabel.text = "Stopped";
            self.sendBattery(text: "Stopped")
            ...
            DJISDKManager.keyManager()?.startListeningForChanges(on: batteryLevelKey!, withListener: self, andUpdate: { (oldValue: DJIKeyedValue?, newValue: DJIKeyedValue?) in
                ...
                self.getBatteryLevelLabel.text = "\(newValue!.unsignedIntegerValue) %"
                self.sendBattery(text: "\(newValue!.unsignedIntegerValue) %")
            })
            
            self.getBatteryLevelLabel.text = "Started...";
            self.sendBattery(text: "Started...")
        }
        isListeningBattery = !isListeningBattery
    }
    
    func sendBattery(text: String) {
        let ref = Database.database().reference().child("battery")
            
            ref.setValue(text, withCompletionBlock:{
                (error, reference) in
                if error != nil {
                    print(error!)
                } else {
        ...
        if isListening {
            ...
            DJISDKManager.keyManager()?.startListeningForChanges(on: locationKey!, withListener: self, andUpdate: { (oldValue: DJIKeyedValue?, newValue: DJIKeyedValue?) in
                if newValue != nil {
                    ...
                    self.sendGPS(text: "Lat: \(aircraftCoordinates.coordinate.latitude) - Long: \(aircraftCoordinates.coordinate.longitude)")
                }
            })
            self.listeningCoordinatesLabel.text = "Started...";
            self.sendGPS(text: "Started...")
        }
        isListening = !isListening
    }
    
    func sendGPS(text: String) {
        let ref = Database.database().reference().child("gps")
